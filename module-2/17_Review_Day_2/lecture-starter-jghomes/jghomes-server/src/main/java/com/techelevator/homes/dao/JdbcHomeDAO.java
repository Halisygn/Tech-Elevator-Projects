...
public class JdbcHomeDAO implements HomeDAO {
    ...
    public List<Home> retrieveHomesForSale() {
        ...
        List<Home> homesForSale = new ArrayList<>();
    ...
    @Override
    public List<Home> retrieveHomesByZipCode(int zipCode) {
        List<Home> homesByZipCode = new ArrayList<>();
        String sql = "SELECT home.*, address.* " +
                "FROM home " +
                "JOIN address ON home.addressid = address.addressid " +
                "WHERE zip = ? ";
        ...
        while (results.next()) {
            Home home = mapRowToHome(results);
            homesByZipCode.add(home);
        }
        return homesByZipCode;
    }

    @Override
    public Home update(Home home) throws HomeNotFoundException {

        String addressSQL = "Update address " +
                "SET addressLine = ?, city = ?, state = ?, zip = ? " +
                "WHERE addressid = ?";
        ...
        jdbcTemplate.update(addressSQL,
                ...
                home.getAddress().getZip(), getAddressId(home.getMlsNumber())
        );

        String homeSQL = "Update home " +
                "SET numberofbathrooms = ?, numberofbedrooms = ?, price = ?, shortdescription = ? " +
                "WHERE mlsnumber = ?";
    ...
    public int getAddressId(String mlsId) {
        String sql = "SELECT address.addressid " +
                "FROM home " +
                "JOIN address ON home.addressid = address.addressid " +
                "WHERE mlsnumber = ?";
