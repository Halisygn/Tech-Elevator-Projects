...
//    }
    ...
    @PreAuthorize("permitAll")
    @RequestMapping(path="/homes/filter", method = RequestMethod.GET)
    public List<Home> getHomesByZipCode(@RequestParam int zip) throws HomeNotFoundException {
        return dao.retrieveHomesByZipCode(zip);
    }

    @RequestMapping(path = "/homes/{mlsNumber}", method = RequestMethod.PUT)
    public Home update(@Valid @RequestBody Home home, @PathVariable String mlsNumber) throws HomeNotFoundException {
        return dao.update(home);
    }
}
