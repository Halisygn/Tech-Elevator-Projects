...
public class Exercises {
    ...
    public List<Double> arrayInt2ListDouble(int[] intArray) {
        List<Double> halfOfNumbers = new ArrayList<Double>();
        for (Integer num : intArray) {
            halfOfNumbers.add(num / 2.0);
        }
        return halfOfNumbers;
    }
    ...
    ** INTERVIEW QUESTION **
    ...
    public List<String> fizzBuzzList(Integer[] integerArray) {
        ...
        for (Integer num : integerArray) {
            if (num % 3 == 0 && num % 5 == 0) {
                newListArray.add("FizzBuzz");
            } else if (num % 5 == 0) {
                newListArray.add("Buzz");
            } else if (num % 3 == 0) {
                newListArray.add("Fizz");
            } else {
                newListArray.add(String.valueOf(num));
            }
        }
        return newListArray;
    }
    ...
    public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
        List<Integer> twoLists = new ArrayList<Integer>();
        ...
        for (int i = 0; i < smallList.size(); i++) {
            twoLists.add(listOne.get(i));
            twoLists.add(listTwo.get(i));
        }
        if (listOne.size() != listTwo.size()) {
            List<Integer> subList = bigList.subList(smallList.size(), bigList.size());
            twoLists.addAll(subList);
        }
        return twoLists;
    }
